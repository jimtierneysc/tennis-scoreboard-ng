...
RSpec::Matchers.define :eql_match do |match|
  match do |json|
    ...
    if match.doubles
      ...
      h.eql_result :winner, json[:winner].nil? || json[:winner] == match.team_winner.id
    else
...
class MatchEql
  ...
  def eql_member member
    eql_result member, json[member] == match.send(member)
  end

  def eql_member_to_s member
    eql_result member, json[member] == match.send(member).to_s
  end

  def eql_team opponent
    eql_result "#{opponent}.id", json[opponent][:id] == match.send(opponent).id
    eql_result "#{opponent}.name", json[opponent][:name] == match.send(opponent).name
  end

  def eql_result member, result
